...
class Birthday extends Base {
	...
	function onBirthDayPoints() {
		...
		if ( apply_filters( 'wlr_sent_birthday_email_based_on_time_zone',
			false )
		) {
			$current_date
				= self::$woocommerce->convert_utc_to_wp_time( $current_date,
				'Y-m-d' );
		}
		...
		foreach ( $birth_users as $birth_user ) {
			...
			if ( ! empty( $birth_user->user_email )
			     && isset( $birth_user->is_banned_user )
			     && $birth_user->is_banned_user == 1
			) {
				continue;
			}
			$action_data = array(
	...
	function onUpdateBirthDay() {
		...
		$display_birth_date = ! empty( $birthdate )
		                      && $birthdate != '0000-00-00'
			? self::$woocommerce->convertDateFormat( $birthdate )
			: ( ! empty( $birthdate )
				? self::$woocommerce->beforeDisplayDate( $birthdate ) : '' );
		$json               = array(
			'success'             => true,
	...
	public function earnViaBirthdateUpdate( $user_email, $birthdate, $campaign_id = 0 ) {
		...
		$already_user
		                  = $birthdate_helper->getPointUserByEmail( $user_email );
		if ( empty( $already_user ) ) {
			$id = 0;
			//Create New User
			$unique_refer_code = $birthdate_helper->get_unique_refer_code( '',
				false, $user_email );
			$_data             = array(
				...
				'birth_date'        => self::$woocommerce->beforeSaveDate( $birthdate,
					'Y-m-d' ),
		...
		$log_data   = array(
			...
			'note'                => sprintf( __( '%s customer birthday updated to %s.',
				'wp-loyalty-rules' ), $user_email, $birthdate ),
			'admin_id'            => isset( $admin_user->ID )
			                         && ! empty( $admin_user->ID )
				? $admin_user->ID : 0,
	...
	function initProSchedule() {
		...
		if ( false === $timestamp ) {
			$scheduled_time = strtotime( '+3 hours',
	...
	function addCustomBirthdayField() {
		...
		$birthday_campaigns
		                     = $earn_campaign_model->getCampaignByAction( 'birthday' );
		if ( empty( $birthday_campaigns ) ) {
			return;
		}
		$options = self::$woocommerce->getOptions( 'wlr_settings', array() );
		if ( empty( $options ) || ! is_array( $options )
		     || empty( $options['birthday_display_place'] )
		     || ! is_string( $options['birthday_display_place'] )
		) {
			return;
		}
		...
		if ( is_admin() ) {
			add_action( 'show_user_profile',
				array( $this, 'addBirthdayInProfile' ) );
		}
	...
	public function getBirthdayDateSchema() {
		...
		return [
			'wlr_dob' => [
				'description' => __( 'Checkout birthday input',
					'wp-loyalty-rules' ),
	...
	function saveBirthdayDate(
		...
		if ( ! isset( $request['extensions'] )
		     || ! isset( $request['extensions']['wlr_checkout_block'] )
		     || ! isset( $request['extensions']['wlr_checkout_block']['wlr_dob'] )
		     || empty( $request['extensions']['wlr_checkout_block']['wlr_dob'] )
		) {
			return;
		}
	...
	private function addInCheckoutForm() {
		...
		$user
		                      = $earn_campaign_helper->getPointUserByEmail( $user_email );
		$birthday_date        = is_object( $user )
		                        && isset( $user->birthday_date )
		                        && ! empty( $user->birthday_date )
		                        && $user->birthday_date != '0000-00-00'
			? $user->birthday_date
			: ( is_object( $user )
			    && isset( $user->birth_date )
			    && ! empty( $user->birth_date )
				? self::$woocommerce->beforeDisplayDate( $user->birth_date,
					"Y-m-d" ) : '' );
		if ( empty( $birthday_date ) || $birthday_date == '0000-00-00' ) {
			...
			add_action( 'woocommerce_after_checkout_validation', array(
				$this,
				'validateBirthdayInCheckoutForm'
			), 10, 2 );
			add_action( 'woocommerce_checkout_create_order',
				array( $this, 'saveBirthdayInCheckoutForm' ), 10, 2 );
			/* Checkout form end*/
		}
	}
	...
	private function addInRegistrationForm() {
		if ( self::$woocommerce->canShowBirthdateField() ) {
			...
			add_action( 'woocommerce_register_form',
				array( $this, 'addBirthdayInRegisterForm' ) );
			add_action( 'woocommerce_register_post',
				array( $this, 'validateBirthdayInRegisterForm' ), 10, 3 );
			add_action( 'woocommerce_created_customer',
				array( $this, 'saveBirthdayInRegisterForm' ), 10, 3 );
	...
	private function addInAccountDetailsPageForm() {
		if ( self::$woocommerce->canShowBirthdateField() ) {
			...
			add_action( 'woocommerce_edit_account_form',
				array( $this, 'addBirthdayInAccountDetailsForm' ) );
			add_action( 'woocommerce_save_account_details_errors', array(
				$this,
				'validateBirthdayInAccountDetailsForm'
			) );
			add_action( 'woocommerce_save_account_details',
				array( $this, 'saveBirthdayInAccountDetailsForm' ) );
			/* Account details form start */
		}
	}
	...
	function addBirthdayInProfile( $user ) {
		...
		if ( empty( $user_data ) || ! is_object( $user_data )
		     || ! isset( $user_data->birth_date )
		) {
			return '';
		}
		$dob           = ! empty( $user_data->birth_date )
			? self::$woocommerce->beforeDisplayDate( $user_data->birth_date )
			: "";
		...
		if ( $validate_date ) {
            ...
            <table class="form-table">
                <tr>
                    <th><label for="birthday_date"><?php _e( 'Date of birth',
								'wp-loyalty-rules' ); ?></label></th>
                    <td>
	...
	static function validateDateORNull( $value ) {
		if ( empty( $value )
		     || in_array( $value, array( '', null, 0, '-' ) )
		) {
			return apply_filters( 'wlr_custom_birthday_validation_check', true,
				$value );
		}
		$is_date = false;
		...
		return apply_filters( 'wlr_custom_birthday_validation_check', $is_date,
			$value );
	}
	...
	function addBirthdayInRegisterForm() {
		woocommerce_form_field( 'wlr_dob', array(
			...
			'required'          => apply_filters( 'wlr_is_required_birthday_registration_field',
				false ),
	...
	function validateBirthdayInRegisterForm( $username, $user_email, $errors ) {
		...
		if ( ! $validate_date ) {
			$errors->add( 'dob_error',
				__( 'Birthday date is required!', 'wp-loyalty-rules' ) );
		}
	...
	public static function getUserBirthday( $billing_email, $dob ) {
		...
		$earn_campaign_helper = EarnCampaign::getInstance();
		$user
		                      = $earn_campaign_helper->getPointUserByEmail( $billing_email );
	...
	function addBirthdayInAccountDetailsForm() {
		...
		woocommerce_form_field( 'wlr_dob', array(
			'label'             => __( 'Birthday', 'wp-loyalty-rules' ),
			'type'              => 'date',
			'required'          => apply_filters( 'wlr_is_required_birthday_account_details_field',
				false ),
			'class'             => array( 'form-row-wide' ),
			'custom_attributes' => array(
				'max' => date( 'Y-m-d' ),
