...
class MenuRenderer {
	...
	public function render() {
		...
		if ( $has_terms ) {
			$parent_terms = $this->data_manager->get_term_children(
				$this->data_manager->get_initial_parent_id(),
				true
			);
	...
	private function render_item( $term, $level ) {
		...
		$disable_links  = ( $term["link"] === "#" || ! $term["link"] )
		                  && $this->settings_manager->get_value( 'link_sharp_setting' ) === true
		                  && ! $this->is_toggled_link( $has_children, $level );
		// Classes
		$class   = "iksm-term";
		$classes = $class;
        ...
        <div class="<?php echo $classes; ?>" data-id="<?php echo $term["id"]; ?>">
            <div class="<?php echo RenderUtils::inner_class( $class, "inner" ); ?>" tabindex='0'>
                <<?php echo $link_tag ?> class="<?php echo $link_class ?>" <?php echo $term_link_attr ?>>
				<?php
				...
				RenderUtils::do_actions( $this->post_id, "item_link", $term, $level );
				?>
            </<?php echo $link_tag ?>>
			<?php
			if ( $has_children ) {
			...
			RenderUtils::do_actions( $this->post_id, "item", $term, $level );
			?>
        </div>
		<?php
		RenderUtils::do_actions( $this->post_id, "after_item", $term, $level );
		if ( $has_children ) {
	...
	private function render_item_shifts( $level ) {
		...
		if ( is_array( $shift_size ) && $level > 1 ) {
            ...
            <div
              ...
              style="<?php echo "width:$width; min-width:$width; max-width:$width;" ?>"></div>
			<?php
		}
	}
	...
	private function render_item_image( $term ) {
		...
		if ( $display ) {
			...
			// Getting image
			if (
				$this->data_manager->is_source_taxonomy()
				|| ( $this->data_manager->is_source_faqs() && $sub_type !== "faq_answer" )
			) {
				if ( $sub_type === "faq_question" ) {
					$image_id          = get_post_thumbnail_id( $term_id );
					$is_images_enabled = true;
				} else if ( Utils::get( $term, "is_post" ) ) {
					$image_id = get_post_thumbnail_id( $term_id );
				} else {
	...
	private function render_item_text( $term ) {
		?>
        <span class="<?php echo RenderUtils::inner_class( "iksm-term", "text" ); ?>"><?php
			echo Utils::get( $term, "sub_type" ) === "faq_answer"
                ? apply_filters( 'the_content', $term["title"] )
                : esc_html( $term["title"] );
			?></span>
		<?php
	}
	...
	private function render_item_posts_count__premium_only( $term ) {
		...
		$showZero = $this->get_setting_value( "display_zero_count" );

		if ( $display ) {
			$count = $term['posts_count'];

			if ( is_int( $count ) ) {
				if ( $count === 0 && $showZero === 'hide' ) {
					return;
				}
	...
	private function render_item_toggle() {
		...
		if ( $display ) {
			...
			if ( $is_class ) {
				$icon_tag = $this->settings_manager->get_value(
					StylesSettingsGenerator::generate_setting_key( "toggle", "main", "icon_tag" )
				);
				if ( empty( $icon_tag ) ) {
					$icon_tag = "i";
				}
			...
			?>
            <div class="<?php echo RenderUtils::inner_class( "iksm-term", "toggle" ); ?>" tabindex="0">
	...
	private function render_data_args() {
		...
		if ( IKSM\iks_menu_fs()->is_plan__premium_only( "pro", true ) ) {
			...
			array_push( $keys, "toggle_by_item_click" );
	...
	private function is_toggled_link( $has_children, $level ) {
		...
		return (
			$has_children && (
				$toggle_by_item_click === true
				|| $disable_parent_links_level === 0 // Hint: 0 means disable all levels
				|| $level <= $disable_parent_links_level
			)
		);
	}
	...
	private function get_link_attr( $term, $has_children, $level ) {
		...
		if ( IKSM\iks_menu_fs()->is_plan__premium_only( "pro", true ) ) {
			if ( $this->is_toggled_link( $has_children, $level ) ) {
				return "data-toggle='1' $common_attrs";
			}
		}
