...
//		Utils::pretty_dump( $this->get_data() );
	...
	private function prepare_initial_parent_id() {
		if ( $this->is_source_taxonomy() ) {
			...
			} else if ( $this->settings_manager->has_setting( "parent" ) ) {
		...
		if ( $taxonomy ) {
			...
			if ( ! ! $this->settings_manager->get_value( "show_current_terms_tree" ) ) {
				...
				if ( $queried_term_id ) {
					...
					if ( count( $sub_terms ) ) { // Displaying entire menu, when no items detected for current element
						// All checks passed: set term ID + un-parent to avoid conflicts
						$this->args['child_of'] = $queried_term_id; // child_of = with sub items
						...
						if ( $show_posts ) {
							if ( isset( $posts_by_terms[ $id ] ) ) {
								...
								foreach ( $result_posts as $post ) {
									$post_id      = $post->ID;
									$result_id    = $this->post_id_prefix . $post_id; // Sometimes WP has the same IDs for terms & posts (for some reason), so it's better to handle this situation
	...
	private function add_element_and_descendants($element, &$filtered_elements, $elements) {
		foreach ($filtered_elements as $filtered_element) { // skip, if element already added
			if ($filtered_element->term_id == $element->term_id) {
				return;
			}
		}
		$filtered_elements[] = $element; // add parent element
		foreach ($elements as $child_element) { // add all children
				...
				if (
					...
					switch ( $ie_type ) {
						...
						case "certain_ids": // Expand with Certain IDs
							if ( ! empty( $ie_ids ) && in_array( $item["initial_id"], $ie_ids, true ) ) {
								$this->process_term_expand__premium_only( $item, $index, false );
							}
							break;
	...
	public function get_queried_object_term_id() {
		return Utils::object_has_property( $this->queried_object, 'term_id' )
			? $this->queried_object->term_id : null;
	}
